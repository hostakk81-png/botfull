...
# Заменяем все старые домены (su, pro, ru) на новый домен pw
sed -i \
...
find "${BASE_DIR}/static" "${BASE_DIR}/img" "${BASE_DIR}/fonts" -type f \( -name "*.css" -o -name "*.js" -o -name "*.woff" -o -name "*.woff2" -o -name "*.ttf" \) -exec sed -i \
  -e "s|https://tgstat\.su|https://${NEW_DOMAIN}|gI" \
  -e "s|https://tgstat\.pro|https://${NEW_DOMAIN}|gI" \
  -e "s|https://tgstat\.ru|https://${NEW_DOMAIN}|gI" \
  -e "s|http://tgstat\.su|https://${NEW_DOMAIN}|gI" \
